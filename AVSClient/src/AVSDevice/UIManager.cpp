...
void UIManager::onRequestAuthorization(const std::string& url, const std::string& code) {
    m_executor.submit([this, url, code]() {
        m_authCheckCounter = 0;
        ThunderLogger::log("NOT YET AUTHORIZED");
        std::ostringstream oss;
        ...
        if (m_authState != newState) {
            ...
            switch (m_authState) {
                ...
                case AuthObserverInterface::State::REFRESHED:
                    ThunderLogger::log("Authorized!");
                    break;
                case AuthObserverInterface::State::EXPIRED:
                    ThunderLogger::log("AUTHORIZATION EXPIRED. RETRYING...");
                    break;
                case AuthObserverInterface::State::UNRECOVERABLE_ERROR:
                    switch (newError) {
                        ...
                        case AuthObserverInterface::Error::INVALID_CBL_CLIENT_ID: {
                            ...
                            ThunderLogger::log({oss.str(), ENTER_LIMITED});
...
void UIManager::printWelcomeScreen() {
    m_executor.submit([]() { ThunderLogger::log(ALEXA_WELCOME_MESSAGE); });
}

void UIManager::printHelpScreen() {
    m_executor.submit([]() { ThunderLogger::log(HELP_MESSAGE); });
}

void UIManager::printLimitedHelp() {
    m_executor.submit(
        [this]() { ThunderLogger::log(LIMITED_HELP_HEADER + m_failureStatus + LIMITED_HELP_MESSAGE); });
}

void UIManager::printSettingsScreen() {
    m_executor.submit([]() { ThunderLogger::log(SETTINGS_MESSAGE); });
}
...
void UIManager::printLocaleScreen() {
    ...
    auto printLocaleMessage = [supportedLocales, supportedLocaleCombinations]() {
        ...
        ThunderLogger::log(LOCALE_MESSAGE_HEADER + optionString + LOCALE_MESSAGE_FOOTER);
    };
...
void UIManager::printSpeakerControlScreen() {
    m_executor.submit([]() { ThunderLogger::log(SPEAKER_CONTROL_MESSAGE); });
}

void UIManager::printFirmwareVersionControlScreen() {
    m_executor.submit([]() { ThunderLogger::log(FIRMWARE_CONTROL_MESSAGE); });
}

void UIManager::printVolumeControlScreen() {
    m_executor.submit([]() { ThunderLogger::log(VOLUME_CONTROL_MESSAGE); });
}

#ifdef ENABLE_PCC
void UIManager::printPhoneControlScreen() {
    m_executor.submit([]() { ThunderLogger::log(PHONE_CONTROL_MESSAGE); });
}

void UIManager::printCallIdScreen() {
    m_executor.submit([]() { ThunderLogger::log(ENTER_CALL_ID_MESSAGE); });
}

void UIManager::printCallerIdScreen() {
    m_executor.submit([]() { ThunderLogger::log(ENTER_CALLER_ID_MESSAGE); });
}
#endif

#ifdef ENABLE_MCC
void UIManager::printMeetingControlScreen() {
    m_executor.submit([]() { ThunderLogger::log(MEETING_CONTROL_MESSAGE); });
}

void UIManager::printSessionIdScreen() {
    m_executor.submit([]() { ThunderLogger::log(ENTER_SESSION_ID_MESSAGE); });
}

void UIManager::printCalendarItemsScreen() {
    m_executor.submit([]() { ThunderLogger::log(ENTER_CALENDAR_ITEMS_FILE_PATH_MESSAGE); });
}
#endif

void UIManager::printCommsControlScreen() {
    m_executor.submit([]() { ThunderLogger::log(COMMS_MESSAGE); });
}

void UIManager::printErrorScreen() {
    m_executor.submit([]() { ThunderLogger::log("Invalid Option"); });
}

void UIManager::microphoneOff() {
    m_executor.submit([]() { ThunderLogger::log("Microphone Off!"); });
}

void UIManager::printResetConfirmation() {
    m_executor.submit([]() { ThunderLogger::log(RESET_CONFIRMATION); });
}

void UIManager::printReauthorizeConfirmation() {
    m_executor.submit([]() { ThunderLogger::log(REAUTHORIZE_CONFIRMATION); });
}

void UIManager::printResetWarning() {
    m_executor.submit([]() { ThunderLogger::log(RESET_WARNING); });
}

void UIManager::printDoNotDisturbScreen() {
    m_executor.submit([]() {
        ThunderLogger::log(DONOTDISTURB_CONFIRMATION_HEADER);
        ThunderLogger::log(ENABLE_SETTING_MENU);
    });
}

void UIManager::printWakeWordConfirmationScreen() {
    m_executor.submit([]() {
        ThunderLogger::log(WAKEWORD_CONFIRMATION_HEADER);
        ThunderLogger::log(ENABLE_SETTING_MENU);
    });
}
...
void UIManager::printSpeechConfirmationScreen() {
    m_executor.submit([]() {
        ThunderLogger::log(SPEECH_CONFIRMATION_HEADER);
        ThunderLogger::log(ENABLE_SETTING_MENU);
    });
}

void UIManager::printTimeZoneScreen() {
    m_executor.submit([]() { ThunderLogger::log(TIMEZONE_SETTING_MENU); });
}
...
void UIManager::onBooleanSettingNotification(
    ...
    m_executor.submit([msg]() { ThunderLogger::log(msg); });
}
...
void UIManager::printCommsNotSupported() {
    m_executor.submit([]() { ThunderLogger::log("Comms is not supported in this device."); });
}
...
void UIManager::onActiveDeviceConnected(const DeviceAttributes& deviceAttributes) {
    m_executor.submit([deviceAttributes]() {
        ...
        ThunderLogger::log({"BLUETOOTH DEVICE CONNECTED", "Name: " + deviceAttributes.name, oss.str()});
    });
}
...
void UIManager::onActiveDeviceDisconnected(const DeviceAttributes& deviceAttributes) {
    m_executor.submit([deviceAttributes]() {
        ...
        ThunderLogger::log({"BLUETOOTH DEVICE DISCONNECTED", "Name: " + deviceAttributes.name, oss.str()});
    });
}
